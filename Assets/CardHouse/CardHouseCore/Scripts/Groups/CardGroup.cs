...
namespace CardHouse
{
    ...
    public class CardGroup : MonoBehaviour
    {
        ...
        {
            HandleTriggerEnter(col.gameObject);
        }

        public void HandleTriggerExit2D(Collider2D col)
        {
            HandleTriggerExit(col.gameObject);
        }
        ...
        public void HandleTriggerEnter(GameObject obj)

        {
            var draggable = obj.GetComponent<DragOperator>();
            if ((draggable == null || draggable.DragAction == DragAction.Mount) && !HasRoom())
                return;

            RespondToObjectCrossingBoundary(true);
        }


        public void HandleTriggerExit(GameObject obj)

        {
            RespondToObjectCrossingBoundary(false);
        }

        void RespondToObjectCrossingBoundary(bool isEntry)
        {
